...
const SectionHeading = ({ title, watermark, animated = true }: SectionHeadingProps) => {
  const parallax = useParallax({
    translateX: animated ? [-200, 200] : [0, 0],
    easing: 'easeInOut'
  })
  ...
  return (
    <div className='relative overflow-hidden text-center section-heading pb-14'>
      ...
      <span
        ref={parallax.ref}
        className='absolute z-0 font-bold uppercase transform -translate-x-1/2 pointer-events-none left-1/2 -top-2 text-9xl text-heading opacity-5'
      >
        {watermark}
      </span>
    </div>
